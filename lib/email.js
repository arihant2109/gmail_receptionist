/**
 * Send email for issues and bookings via Resend HTTP API.
...
async function sendEmail({ to, subject, body }) {
  const emailTo = to && String(to).trim();
  if (!emailTo) {
    console.warn('Email not sent: recipient (to) is missing');
    return { ok: false, error: 'Recipient address (to) is required' };
  }
